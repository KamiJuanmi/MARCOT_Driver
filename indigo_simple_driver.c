...
static void hucha_timer_callback(indigo_device *device)
{
    if (PRIVATE_DATA->current_dinerico < PRIVATE_DATA->target_dinerico)
    {
        ...
        indigo_reschedule_timer(device, AHORRA_TIME, &PRIVATE_DATA->hucha_timer);
    }
    else
    {
...
static indigo_result hucha_attach(indigo_device *device)
{
    ...
    if (indigo_device_attach(device, DRIVER_NAME, DRIVER_VERSION, 0) == INDIGO_OK)
    {
        ...
        HUCHA_DINERO_PROPERTY = indigo_init_number_property(HUCHA_DINERO_PROPERTY, device->name, "HUCHA_DINERO", "Mi grupo", "Ahorra", INDIGO_OK_STATE, INDIGO_RW_PERM, 1);
        if (HUCHA_DINERO_PROPERTY == NULL)
            return INDIGO_FAILED;
        indigo_init_number_item(HUCHA_DINERO_ITEM, "DINERO", "Ahorra", 0, 10000, AHORRA_SPEED, 10);
        PRIVATE_DATA->current_dinerico = 10;
...
static indigo_result hucha_change_property(indigo_device *device, indigo_client *client, indigo_property *property)
{
    ...
    else if (indigo_property_match(HUCHA_DINERO_PROPERTY, property))
    {
        ...
        HUCHA_DINERO_PROPERTY->state = INDIGO_BUSY_STATE;
        PRIVATE_DATA->target_dinerico = HUCHA_DINERO_ITEM->number.value;
		HUCHA_DINERO_ITEM->number.value = PRIVATE_DATA->current_dinerico;
        indigo_update_property(device, HUCHA_DINERO_PROPERTY, "Target dinero %g", PRIVATE_DATA->target_dinerico);
        indigo_set_timer(device, AHORRA_TIME, hucha_timer_callback, &PRIVATE_DATA->hucha_timer);
        return INDIGO_OK;
    }
